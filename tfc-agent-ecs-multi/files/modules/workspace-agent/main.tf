...
resource "tfe_workspace" "ws-agent-template" {
...
resource "tfe_variable" "env_vars" {
  ...
  value        = "1"
  category     = "env"
  workspace_id = var.identifier != "" ? tfe_workspace.ws-agent-template[0].id : tfe_workspace.ws-novcs[0].id
  depends_on   = [tfe_workspace.ws-agent-template,tfe_workspace.ws-novcs]
}
